...
async function onPageLoad() {
  try {
    ...
    getRacers().then((racers) => {
      const html = renderRacerCars(racers); // list of racer cards
...
async function handleCreateRace() {
  const selectedTrack = `Track${store.track_id}`;
...
async function runCountdown() {
  try {
    ...
    return new Promise((resolve) => {
      ...
      const counter = setInterval(decrementCounter, 1000);
